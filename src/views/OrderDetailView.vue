...
export default {
  ...
  methods: {
    async fetchOrder() {
      try {
        const res = await axios.get(`http://localhost:5000/orders/${this.$route.params.id}`);
        this.order = res.data;
      } catch (err) {
    ...
    async updateStatus(newStatus) {
      try {
        await axios.patch(`http://localhost:5000/orders/${this.order._id}`, { status: newStatus });
    ...
    async deleteOrder() {
      try {
        await axios.delete(`http://localhost:5000/orders/${this.order._id}`);
